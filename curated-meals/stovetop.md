# Curated Meals — Stovetop

5 meals that can be cooked stovetop. Some are multi-method (also available via slow cooker, oven, or pressure cooker) — only the stovetop method is shown here.

For multi-method meals, the same plates are available regardless of which method cooked the base.

Method equipment requirements are listed per method. The user must have ALL listed equipment. If they don't, skip the method (try another method of the same meal, or a different meal).

---

## butter_chicken — Butter Chicken
- Cuisine: indian
- Primary protein: chicken
- Eligible slots: lunch, dinner, second_lunch, early_dinner
- Scale: 0.7–1.5
- Produces servings: 6
- Allergens: Dairy
- Also available in: slow-cooker.md, pressure-cooker.md

### Stovetop scratch method (`stovetop_scratch`)
- Equipment: stovetop
- Active 25 min, total 35 min, skill 3, scratch

### Stovetop shortcut method (`stovetop_shortcut`)
- Equipment: stovetop
- Active 12 min, total 20 min, skill 2, uses Patak's jar sauce

### Plates
- `standard` — Butter Chicken with Basmati Rice
  - Macros: 720 kcal / 48g P / 65g C / 28g F / 4g fiber
  - Assembly: 0 min, equipment: stovetop (for rice)

---

## bolognese — Slow-Simmered Bolognese
- Cuisine: italian
- Primary protein: beef
- Eligible slots: lunch, dinner, second_lunch, early_dinner
- Scale: 0.7–1.5
- Produces servings: 8
- Allergens: Dairy, Eggs, Gluten/Wheat
- Also available in: slow-cooker.md

### Stovetop method (`stovetop`)
- Equipment: stovetop
- Active 20 min, total 140 min, skill 2

### Plates
- `spaghetti` — Spaghetti Bolognese
  - Macros: 996 kcal / 48g P / 97g C / 46g F / 8g fiber
  - Assembly: 12 min, equipment: stovetop
- `baked_potato` — Loaded Bolognese Baked Potato
  - Macros: 1007 kcal / 44g P / 84g C / 55g F / 12g fiber
  - Assembly: 65 min, equipment: oven
- `garlic_bread` — Bolognese with Garlic Bread
  - Macros: 968 kcal / 42g P / 65g C / 59g F / 7g fiber
  - Assembly: 18 min, equipment: oven
- `lasagne` — Bolognese Lasagne ⚡ STUNT
  - Macros: 1130 kcal / 62g P / 76g C / 63g F / 7g fiber
  - Assembly: 70 min, equipment: oven + stovetop
  - Stunt plate. Reserve note: uses 6 base servings in one bake; the bake yields 6 portions. Plan around the commitment.

---

## massaman — Slow-Cooked Massaman Beef Curry
- Cuisine: thai
- Primary protein: beef
- Eligible slots: lunch, dinner, second_lunch, early_dinner
- Scale: 0.7–1.5
- Produces servings: 8
- Allergens: Fish, Shellfish, Nuts
- Also available in: slow-cooker.md

### Stovetop method (`stovetop`)
- Equipment: stovetop
- Active 20 min, total 140 min, skill 2

### Plates
- `rice` — Massaman Beef Curry with Rice
  - Macros: 1403 kcal / 59g P / 133g C / 68g F / 7g fiber
  - Assembly: 15 min, equipment: stovetop

---

## chilli_con_carne — Slow-Simmered Chilli Con Carne
- Cuisine: mexican
- Primary protein: beef
- Eligible slots: lunch, dinner, second_lunch, early_dinner
- Scale: 0.7–1.5
- Produces servings: 8
- Allergens: Dairy
- Also available in: slow-cooker.md

### Stovetop method (`stovetop`)
- Equipment: stovetop
- Active 15 min, total 105 min (up to 150 min for deeper flavour), skill 2

### Plates
- `bowl` — Bulking Chilli Bowl
  - Macros: 1263 kcal / 52g P / 123g C / 62g F / 18g fiber
  - Assembly: 15 min, equipment: stovetop
- `nachos` — Loaded Chilli Nachos ⚡ STUNT
  - Macros: 1319 kcal / 53g P / 94g C / 82g F / 20g fiber
  - Assembly: 18 min, equipment: oven
  - Stunt plate. Saturday-night positioning.
