...
Some plates require their own equipment beyond the base method (`equipment_required` field on the plate entry). A plate needing oven assembly is unavailable to a user without oven, even if they have the method's cooking equipment. Filter plates by user's equipment after filtering methods.
