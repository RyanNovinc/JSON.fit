...
6. **Session Duration Reporting** — Calculate and report the duration of each training day with this exact formula, using the rest figures from the plan's rest summary: `(sets × 45 s) + (sets × rest) + (exercises × 150 s for setup and moving between stations) + 300 s warm-up`. The JSON step uses the same formula, so the figure the user approves here is the figure that lands in their file. A set-and-rest-only estimate runs about 25-30 minutes short per session and hides sessions that are really over 2 hours. Include duration in the program output so the user knows what to expect. Do NOT treat duration as a constraint to fix — the user's volume and rest preferences drive session length, and that is intentional.
...
- **Priority placement**: If the profile names priority muscles, no priority muscle is left to the end of a long session. Two rules, and the first one wins when they collide:
  1. **Never pre-fatigue a helper muscle.** An isolation exercise for a muscle never sits ahead of a compound in the same session that tags that muscle as Secondary. Curls go after the last row, pulldown or pull-up. Triceps isolation goes after the last press. Tired biceps cut a pulldown short before the lats have done their work.
  2. **Then place priority isolation early.** A priority muscle's isolation work goes straight after the last compound that tags it, ahead of any non-priority exercise. If no compound in that session tags it (side delts on a pull day, for example), it may open the session or follow the first compound.
  A priority muscle counts as trained from the first exercise that tags it, Primary or Secondary. Curls placed before a pulling exercise, or priority isolation parked behind non-priority work, is a FAIL. Fix it by reordering only — no set count changes, so volume is unaffected.
